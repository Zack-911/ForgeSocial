...
import { IncludeGroups } from '../../utils/spotifyEnums';
...
export default new NativeFunction({
  ...
  async execute(ctx, [id, includeGroups, market, limit, offset]) {
    ...
    if (includeGroups != null) params.append('include_groups', includeGroups.toString());
    ...
    try {
      const json = await spotifyFetch(
        `/artists/${id}/albums${queryString ? '?' + queryString : ''}`,
        token,
        'GET',
      );
