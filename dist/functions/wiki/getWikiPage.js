...
exports.default = new forgescript_1.NativeFunction({
    ...
    async execute(ctx, [subreddit, page]) {
        ...
        if (page) {
            let json = await (0, redditFetch_1.redditFetch)(`r/${subreddit}/wiki/${page}.json`, token, username);
            return this.success(JSON.stringify(json, null, 2));
        }
        else {
            let json = await (0, redditFetch_1.redditFetch)(`r/${subreddit}/wiki/pages.json`, token, username);
            return this.success(JSON.stringify(json, null, 2));
        }
    }
});
