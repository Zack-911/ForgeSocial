...
function functionsToMarkdown(functions) {
    ...
    for (const fn of functions) {
        ...
        if (fn.output)
            md += `- **Output:** ${fn.output.join(', ')}\n`;
        if (fn.brackets !== undefined)
            md += `- **Brackets:** ${fn.brackets}\n`;
...
function generateAllMarkdownDocs() {
    // Events
    const events = JSON.parse(fs_1.default.readFileSync(path_1.default.join(metadataDir, 'events.json'), 'utf8'));
    writeMarkdown('events.md', eventsToMarkdown(events));
    // Functions
    const functions = JSON.parse(fs_1.default.readFileSync(path_1.default.join(metadataDir, 'functions.json'), 'utf8'));
    writeMarkdown('functions.md', functionsToMarkdown(functions));
    // Enums
    const enums = JSON.parse(fs_1.default.readFileSync(path_1.default.join(metadataDir, 'enums.json'), 'utf8'));
