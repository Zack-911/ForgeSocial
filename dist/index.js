...
class ForgeSocial extends forgescript_1.ForgeExtension {
  ...
  async init(client) {
    ...
    if (this.options.github) {
      ...
      forgescript_1.Logger.warn(
        'ForgeSocial: Missing GitHub token. Skipping GitHub initialization.',
      );
    }
    ...
    if (this.options.youtube?.enabled) {
      this.youtube = await youtubei_js_1.Innertube.create({
        ...
        cache: shouldCache
          ? new youtubei_js_1.UniversalCache(true, './ForgeSocial/youtube-cache')
          : undefined,
      });
  ...
  async getAccessToken() {
    ...
    if (!allProvided) {
      forgescript_1.Logger.warn(
        'ForgeSocial: Missing Reddit credentials. Returning null for access token.',
      );
      return null;
    }
  ...
  startPolling() {
    ...
    if (this.accessToken && this.options.reddit?.redditUsername) {
      (0, pollSubreddit_1.startPollingTrackedSubreddits)(
        this.accessToken,
    ...
    // YouTube polling
    (0, pollYoutube_1.startPollingTrackedChannels)(this, (video) =>
